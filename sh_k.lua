--addons/core-octolib/lua/octolib/modules/_client/skin.lua
...
local SKIN = {}
SKIN.PrintName = 'Provenance Derma Skin'
SKIN.Author = 'T1NTINY x chelog (спасибо)'
...
SKIN.Colours.Tree = {}
SKIN.Colours.Tree.Hover =  Color(70,70,70,255)
...
function SKIN:PaintPanel(pnl, w, h)
	...
	draw.RoundedBox(4, 1, 1, w-2, h-2,  Color(70,70,70,255))
...
function SKIN:PaintButton(pnl, w, h)
	...
	if pnl.Depressed then
		draw.RoundedBox(4, 0, off, w, h-off,  Color(70,70,70,255))
		draw.RoundedBox(4, 0, off, w, h-off,  Color(70,70,70,255))
	else
		draw.RoundedBox(4, 0, 0, w, h,  Color(70,70,70,255))
		draw.RoundedBox(4, 0, 0, w, h-off,  Color(70,70,70,255))
...
function SKIN:PaintTreeNodeButton( pnl, w, h )
	...
	draw.RoundedBox(4, 38, 2, w + 6, h-3,  Color(70,70,70,255))
...
function SKIN:PaintCategoryButton( panel, w, h )
	...
	if ( panel.AltLine ) then
		...
		if ( panel.Depressed or panel.m_bSelected ) then draw.RoundedBox(4, 0, 0, w, h,  Color(70,70,70,140))
		elseif ( panel.Hovered ) then draw.RoundedBox(4, 0, 0, w, h,  Color(70,70,70,140))
...
function SKIN:PaintSliderKnob( pnl, w, h )
	...
	if pnl:GetDisabled() then
		return draw.SimpleText(utf8.char(0xf111), 'dbg-icons', w/2, h/2,  Color(70,70,70,140), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
function SKIN:PaintTab(pnl, w, h)
	...
	if pnl:IsActive() then
		draw.RoundedBoxEx(4, 2, 0, w-5, h, Color(70,70,70,140), true, true, false, false)
	else
...
function SKIN:PaintPropertySheet(pnl, w, h)
	...
	draw.RoundedBox(4, 0, 2, w, h-2,  Color(70,70,70,140))
...
function SKIN:PaintCollapsibleCategory(pnl, w, h)
	...
	draw.RoundedBox(4, 0, 5, w, h-5,  Color(70,70,70,140))
	draw.RoundedBox(4, 0, 0, w, 20,  Color(70,70,70,140))
...
function SKIN:PaintWindowCloseButton(pnl, w, h)
	...
	if pnl.Depressed then
		draw.RoundedBox(8, w/2-8, h/2-7, 16, 16,  Color(205,70,70,140))
		if pnl.Hovered then
			draw.SimpleText(utf8.char(0xf00d), 'dbg-icons', w / 2, h / 2, Color(0,0,0, 120), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
	else
		draw.RoundedBox(8, w/2-8, h/2-7, 16, 16,  Color(205,70,70,150))
		draw.RoundedBox(8, w/2-8, h/2-8, 16, 16,  Color(205,70,70,150))
		if pnl.Hovered then
...
function SKIN:PaintTooltip( pnl, w, h )
	...
	shadow(-7, -4, w + 14, h + 8)
	draw.RoundedBox(4, -3, 0, w + 6, h,  Color(70,70,70,140))
	draw.NoTexture()
	surface.DrawPoly({
		{x = w/2 - 5, y = h},
		{x = w/2 + 5, y = h},
...
function SKIN:PaintScrollBarGrip( panel, w, h )
	...
	draw.RoundedBox(4, w/2-4, h, 0, 8, Color(70,70,70,140))

	if ( panel:GetDisabled() ) then
		return draw.RoundedBox(4, w/2-4, 0, 8, h,  Color(70,70,70,140))
	end
...
function SKIN:PaintMenu( pnl, w, h )
	...
	draw.RoundedBox(4, 0, 0, w, h,  Color(70,70,70,140))
	draw.RoundedBox(4, 1, 1, w-2, h-2,  Color(70,70,70,140))

end

function SKIN:PaintMenuSpacer( pnl, w, h )

	surface.SetDrawColor( Color(70,70,70,140))
...
function SKIN:PaintMenuBar( pnl, w, h )
	...
	draw.RoundedBox(0, 0, 0, w, h,  Color(70,70,70,140))
...
function SKIN:PaintExpandButton( pnl, w, h )
	...
	if pnl:GetExpanded() then
		...
		draw.SimpleText(utf8.char(0xf111), 'dbg-icons', w/2, h/2,  Color(70,70,70,140), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
function SKIN:PaintProgress(pnl, w, h)
	...
	if fr > 0 then
		draw.RoundedBox(8, 1, y + 1, (w-18) * fr + 16, 16,  Color(70,70,70,140))
	end
